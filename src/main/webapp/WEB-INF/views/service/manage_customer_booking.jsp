...
<%--                    <td>${booking.sex}</td>--%>
                    ...
                    <td>
                        <c:choose>
                        <c:when test="${booking.bookingStatus != null}">
                        <c:choose>
                            <c:when test="${booking.bookingStatus == 0}">Dịch vụ đã bị hủy</c:when>
                        <c:when test="${booking.bookingStatus == 1}">Đang xử lý...</c:when>
                        <c:when test="${booking.bookingStatus == 2}">Đặt Thành Công</c:when>
                ...
                <td>Không có thông tin</td>
                    ...
                    <td>
                        <c:if test="${booking.bookingStatus == 1 || booking.billStatus == 2}">
                            <form action="UpdateBookingCustomerID" method="get" onsubmit="return canModifyBooking('${booking.bookingDate}', '${booking.slot}');">
                                <input type="hidden" name="bookingID" value="${booking.bookingID}">
